...
public final class ListUtils {
	...
	//////
	public Object[][] toMultiArrayNumberParams(ResourceData resourceData, CoordinatesData coordinatesData, List<?>... list) {
		if ((list == null) || (list.length == 0)) {
			// TODO Develop Custom Exception
			//throw new RuntimeException(ARRAY_LISTS_ERROR);
			throw new GeneralCustomException(ARRAY_LISTS_ERROR);
		...
		Object[][] array = new Object[list[0].size()][list.length + 2];
		for (int i = 0; i < list[0].size(); i++) {
			array[i][0] = resourceData;
			array[i][1] = coordinatesData;
			//array[i][columnsNumber - 1] = list.get(i);
			for (int j = 0; j < list.length; j++)
				array[i][j+2] = list[j].get(i);
		}
		return array;
	}
	...
	//////
	public Object[][] toMultiArrayNumberParams(List<?>... list) {
		if ((list == null) || (list.length == 0)) {
			// TODO Develop Custom Exception
			//throw new RuntimeException(ARRAY_LISTS_ERROR);
			throw new GeneralCustomException(ARRAY_LISTS_ERROR);
		...
		Object[][] array = new Object[list[0].size()][list.length];
		for (int i = 0; i < list[0].size(); i++) {
			for (int j = 0; j < list.length; j++)
