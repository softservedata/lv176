...
public class SearchByLastNameTest {
	...
	@DataProvider
	public Object[][] getLastNames(ITestContext context) {
		return ListUtils.get().toMultiArrayNumberParams(
				...
				SearchDataRepository.get().getLastNamesFromCVS());
	}

	@Test(dataProvider = "getLastNames")
	public void searchByLastNameTest(ApplicationSources app, String str) {
		aasp = aasp.searhByLastName(str);
		FlexAssert.get()
			.forElement(aasp.getLastNamesTableList())
				.containsText(str)
		.next().check();
	}
}
