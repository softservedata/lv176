...
public class SearchByEmailTest {
	...
	@DataProvider
    public Object[][] getEmails(ITestContext context) {
		return ListUtils.get().toMultiArrayNumberParams(
    			ParameterUtils.get().updateAllApplicationSources(
        				ApplicationSourcesRepository.get()
        					...
        					SearchDataRepository.get().getEmailsFromCVS());
    }
	
	@Test (dataProvider = "getEmails")
	public void searchByEmailTest(ApplicationSources app, String str){
		aasp = aasp.searhByEmail(str);
		FlexAssert.get()
			.forElement(aasp.getEmailsTableList())
				.containsText(str)
		.next().check();
	}
