...
public final class UserUtils {
	...
	public List<IUser> getAllRegistartors() {
		return getAllRegistartors(this.getClass().getResource(filename).getPath().substring(1));
	}
	
	public List<IUser> getAllCoOwners() {
		return getAllCoOwners(this.getClass().getResource(filename).getPath().substring(1));
	}
	...
	public List<IUser> getAllRegistartors(String absoluteFilePath) {
		List<IUser> users = new ArrayList<IUser>();
		for (List<String> row : externalData.getAllCells(absoluteFilePath)) {
			if (row.get(4).toLowerCase().contains("phonenumber")
					|| row.get(10).toLowerCase().contains("data")) {
			...
			if (row.get(7).toLowerCase().equals("registrator")){
				users.add(
						User.get()
						.setPerson(Person.get()
										.setFirstname(row.get(0))
	...
	public List<IUser> getAllCoOwners(String absoluteFilePath) {
		List<IUser> users = new ArrayList<IUser>();
		for (List<String> row : externalData.getAllCells(absoluteFilePath)) {
			if (row.get(4).toLowerCase().contains("phonenumber")
					|| row.get(10).toLowerCase().contains("data")) {
			...
			if (row.get(7).toLowerCase().equals("co-owner")){
				users.add(
						User.get()
						.setPerson(Person.get()
										.setFirstname(row.get(0))
